...
from MAIN.ChebNetII_MME import ChebNetII_MME
...
def main(args): 
    ...
    for i, (train_index, test_index) in enumerate(skf.split(meta.index, meta)) :
        ...
        if args.pnet : 
            ...
            elif args.model == 'ChebNetII' :
                model = ChebNetII_MME(MME_input_shapes , encoder_dims, args.latent_dim , args.decoder_dim , args.h_feats, len(meta.unique()), args.dropout, args.encoder_dropout, cheb_k=args.cheb_order, PNet=net).to(device)
        else :
            if args.model == 'GCN' : 
                ...
                model = ChebNetII_MME(MME_input_shapes , encoder_dims, args.latent_dim , args.decoder_dim , args.h_feats, len(meta.unique()), args.dropout, args.encoder_dropout, cheb_k=args.cheb_order).to(device)
        ...
        if getattr(model, 'uses_full_graph', False):
            test_output_metrics = evaluate_full_graph(model, g, test_index)
        else:
...
def construct_parser():
    ...
    parser.add_argument('--model', type=str, default='GCN', help='Name of Model to instantiate.'
                        'Choose from [GCN, GSage, GAT, ChebNetII]')
    parser.add_argument('--cheb-order', type=int, default=2, help='Chebyshev polynomial order for ChebNetII (default: 2)')
    return parser
