...
def train_full_graph(g, train_index, device, model, labels, epochs, lr, patience,
                     weight_decay=1e-4, step_size=50, gamma=0.5):
    """Full-graph training path used by the ChebNet update."""
    loss_fcn = nn.CrossEntropyLoss()
    optimizer = optim.Adam(model.parameters(), lr=lr, weight_decay=weight_decay)
    scheduler = optim.lr_scheduler.StepLR(optimizer, step_size=step_size, gamma=gamma)

    train_index = torch.as_tensor(train_index, dtype=torch.long, device=device)
    targets = labels.argmax(dim=1).to(device) if labels.ndim > 1 else labels.to(device)
    best_loss = float('inf')
    no_improve = 0
    train_loss = []
    ...
    for epoch in range(epochs):
        model.train()
        optimizer.zero_grad()
        logits = model(g.ndata['feat'], g)
        loss = loss_fcn(logits[train_index], targets[train_index])
        loss.backward()
        optimizer.step()
        scheduler.step()
        ...
        if epoch % 5 == 0:
            pred = logits[train_index].argmax(dim=1)
            acc = (pred == targets[train_index]).float().mean().item()
            print(f"Epoch {epoch:05d} | Loss {loss.item():.4f} | Train Acc. {acc:.4f} | " )
...
def evaluate_full_graph(model, g, index):
    """Evaluate a full-graph model on a subset of nodes."""
    loss_fcn = nn.CrossEntropyLoss()
    model.eval()
    index = torch.as_tensor(index, dtype=torch.long, device=g.device)
    with torch.no_grad():
        logits = model(g.ndata['feat'], g)
        y = g.ndata['label']
        true = y.argmax(dim=1) if y.ndim > 1 else y
        loss = loss_fcn(logits[index], true[index])
        acc = (logits[index].argmax(dim=1) == true[index]).float().mean().item()

        logits_out = logits[index].detach().cpu().numpy()
        binary_out = (logits_out == logits_out.max(1).reshape(-1,1))*1
        labels_out = y[index].detach().cpu().numpy()
        ...
        F1 = 2*((PRC*SNS)/(PRC+SNS)) if (PRC + SNS) > 0 else 0.0
        return loss, acc, F1, PRC, SNS, logits[index], y[index]
...
def train(g, train_index, device ,  model , labels , epochs , lr , patience, n_node_samples=-1, batch_size=1024, weight_decay=1e-4, step_size=50, gamma=0.5, inductive = False, pretrain = False , pnet=False ):
    ...
    """    
    if getattr(model, 'uses_full_graph', False):
        return train_full_graph(g, train_index, device, model, labels, epochs, lr, patience, weight_decay, step_size, gamma)
